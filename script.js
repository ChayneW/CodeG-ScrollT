...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("load", () => {
	  ...
	  // Wait for fade transition, then show main content
	  setTimeout(() => {
	    gsap.to(loader, {
	      opacity: 0,
	      duration: .3,
	  ...
	  }, 3000); // ⏱ stays visible for 3 seconds total
  });
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      if (maskContainer && maskImage) {
      	...
      	// const moveIn = gsap.utils.mapRange(0.25, 0.75, 100, 0, progress);
  		// gsap.set(maskContainer, { left: `${moveIn}vw` });
